...
import temportalist.esotericraft.sorcery.client.EnumKeyAction
import temportalist.esotericraft.sorcery.common.network.{PacketKeyPressed, SyncSorceryPlayerHandler}
		...
		action match {
			case EnumKeyAction.CAST =>
				...
				if (status == EnumStatus.OK) {
					//SorceryPlayer.get(player).setSleepStart(spawn)
				}
