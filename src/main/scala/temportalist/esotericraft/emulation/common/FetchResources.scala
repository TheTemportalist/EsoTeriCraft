		...
		override def run(): Unit = {
			...
			if (mappings != null) for (entry <- mappings) {
				...
				try {
					...
					case e: Exception =>
						//Emulation.log("[Abilities] Entity class \'" + entry._1 + "\' not found. Skipping.")
				}
				if (classKey != null) {
					if (!classOf[EntityLivingBase].isAssignableFrom(classKey)) {
						Emulation.log("[Abilities] Entity class in ability mappings named " +
						...
						try {
							...
							entry._2.asInstanceOf[AnyRef] match {
								...
								case _ =>
									Emulation.log("[Abilities] Error, object not calculated from json " + entry._2.getClass.getCanonicalName)
							}
							for (property <- properties) {
								...
								Emulation.createAbility(abilityName, argStrings, property) match {
