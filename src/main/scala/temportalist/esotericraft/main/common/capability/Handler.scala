...
import temportalist.origin.api.common.capability.{CapSerializable, ICapability, StorageBlank}
...
abstract class Handler[E, N <: NBTBase, T <: ICapability[E, N]](
	...
	def register(mod: IModPlugin, key: String): Unit = {
		...
		CapabilityManager.INSTANCE.register(
			...
			new StorageBlank[T],
			new Callable[T] {
				override def call(): T = getNewCapabilityInstance
			}
		)
