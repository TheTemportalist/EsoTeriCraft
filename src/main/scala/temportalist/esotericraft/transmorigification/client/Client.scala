...
object Client extends IPluginClient with IKeyBinderOwner {
	...
	override def preInit(): Unit = {
		...
		Transform.registerHandler(OverlaySidebarMorph)

	}

	@SideOnly(Side.CLIENT)
	override def getKeyBinders: Seq[IKeyBinder] = Seq(ModKeys)
	...
	object ModKeys extends IKeyBinder {
		...
		override def register(): Unit = {
			...
			this.sidebarLeft = new KeyBinding(prefix + "Left",
				cxt, km, Keyboard.KEY_LBRACKET, cate)
			this.registerKeyBinding(this.sidebarLeft)
			this.sidebarRight = new KeyBinding(prefix + "Right",
				cxt, km, Keyboard.KEY_RBRACKET, cate)
		...
		def onKeyPress(event: KeyInputEvent): Unit = {
			if (Keyboard.isKeyDown(Keyboard.KEY_RETURN)) {
				if (OverlaySidebarMorph.doShowSelector) {
					...
					new PacketSetModel(selectedIndex, 0).sendToServer(Transform)
				...
				if (OverlaySidebarMorph.doShowSelector) {
					val selectedIndex = OverlaySidebarMorph.selectorSelected
					new PacketSetModel(selectedIndex, 1).sendToServer(Transform)
				}
			}
		}
